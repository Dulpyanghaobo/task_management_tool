...
//                    GoalTypeSelectionView()
            ...
            // Custom Tab Bar
            CustomTabBar(selectedTab: $selectedTab).padding(.horizontal)
...
struct CustomTabBar: View {
    ...
    var body: some View {
        HStack {
            ...
            TabBarButton(icon: "person.2.fill", selectedTab: $selectedTab, assignedTab: "chat").offset(x: 8)
    ...
    var body: some View {
        Button(action: {
            ...
            Image(systemName: icon)
                .font(.system(size: 24))
                .foregroundColor(selectedTab == assignedTab ? AppColors.tabbarTintColor : .gray)
...
struct CenterTabBarButton: View {
    ...
    var body: some View {
            Button(action: {
                viewRouter.currentPage = "ActionGenerateView"
            }) {
                Image(systemName: "poweroutlet.type.f.fill")
                    .font(.system(size: 20))
                    .foregroundColor(.white)
                    .padding()
                    .background(Circle().fill(AppColors.tabbarTintColor))
                    .overlay(
                        ZStack {
                            ForEach(0..<3) { index in
                                Circle()
                                    .stroke(AppColors.tabbarTintColor.opacity(0.6), lineWidth: 3)
